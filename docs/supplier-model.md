...
| **Own printed** (instruction cards) | None | `report_only` | Lead time 2 weeks. Two versions confirmed (Next Day Flyers, 48HourPrint); the Basic-kit card is unresolved (PL-4). |
| **SAM Medical** (SAM XT tourniquets) | None | `report_only` | Bought direct, not through NAR. Lead time 2 weeks. |
| **Orca Tactical Gear** (Coyote/Multicam pouches) | None | `report_only` | Replaces World Richman for those two colourways, whose MOQ made them unviable. Lead time and part numbers unknown (PL-1); placeholders `TBD-ORCA-COYOTE` / `TBD-ORCA-MULTICAM`. |
